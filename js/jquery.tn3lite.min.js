...
i.hide();break;case "play":i.click(function(k){if(b.timer.runs){b.timer.stop();b.config.autoplay=false;i.removeClass(b.config.cssID+"-play-active");i.attr("title",b.t("Iniciar presentación"))}else{b.timer.start();b.config.autoplay=true;i.addClass(b.config.cssID+"-play-active");i.attr("title",b.t("Detener presentación"))}k.stopPropagation()});i.attr("title",b.t("Iniciar presentación"));b.config.autoplay&&i.click();break;case "albums":b.albums=new f.fn.tn3.Albums(b.data,i,b.config.cssID);i.hide();i.bind("albums_binit",
...
case "albums-close":b.albums&&b.albums.setControl("close",i);i.attr("title",b.t("Cerrar"));break;case "show-albums":i.click(function(k){b.items.albums&&b.albums.show(0,b.cAlbum,false,true);k.stopPropagation()});i.attr("title",b.t("Album List"));break;case "fullscreen":i.click(function(k){b.fullscreen();k.stopPropagation()});i.attr("title",b.t("Maximizar"))}});if(this.config.width!==null||this.config.height!==null){if(this.config.width==null)this.config.width=this.config.initValues.width;if(this.config.height==
